...
namespace 의료IT공학과.데이터베이스
{
    class SUGANG_DB_Table_Create
    {
        static void Main(string[] args)
        {
            ...
            xRemoteDB db = new xRemoteDB("http://localhost:8080");
        ...
        //----------------------------------------------------
        static int dropTable(xRemoteDB db, string tableName)
        { 
            string queryStr = string.Format("DROP TABLE IF EXISTS {0}", tableName);
        ...
        static int createTable(xRemoteDB db, string tableName, string tableDesign)
        {

            string queryStr = string.Format("CREATE TABLE {0} ({1}) ENGINE=InnoDB DEFAULT CHARSET=utf8", tableName, tableDesign);
        ...
        //------------------------------------------------------
        static int insertRow(xRemoteDB db, string tableName, string dataStr)
        {
        ...
        //------------------------------------------------------
        static void dropTables(xRemoteDB db)
        {
            int cnt = 0;
        ...
        //-------------------------------------------------------
        static void createTables(xRemoteDB db)
        {
            int cnt = 0;

            cnt += createTable(db, "xSTUDENT_STATUS",
                                @"xStatus_code        int         not null, 
                                  xStatus_title       varchar(5)  not null,
        ...
        //-------------------------------------------------
        static void insertRows(xRemoteDB db)
        {
            int cnt = 0;
			//(db, "테이블명", "데이터 문자열")
			...
			//xStudents ---------------------------------------------
			cnt += insertStudent(db, "19615001", "강만기", "kanxxA1234@", "615", 0, 
		...
		//-----------------------------------------------
		static int insertStudent(xRemoteDB db, string hakbun, string name, string pass, string dept, int status, string addr, string email, string phone)
		{
            hakbun = hakbun.Trim();
        ...
        //---------------------------------------------------
        static bool IsNewHakbun(xRemoteDB db, string hakbun)
        {
            string query = string.Format("SELECT xHakbun FROM xSTUDENTS WHERE xHakbun='{0}'", hakbun);

            if (!DB_Query(db, query)) return false;
        ...
        //-----------------------------------------------
        static bool IsValid_IDENTIFIER(xRemoteDB db, string id)
        {
		...
		//-----------------------------------------------
		static bool IsValidDeptCode(xRemoteDB db, string code)
		{
